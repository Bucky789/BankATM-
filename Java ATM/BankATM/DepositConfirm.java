...
public class DepositConfirm extends JFrame {
	/**
	 * 
	 */
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					DepositConfirm frame = new DepositConfirm();
	...
	public DepositConfirm() {
		...
		JLabel lblNewLabel = new JLabel("Entered Amount is :₹"+c.vald());
		lblNewLabel.setFont(new Font("Segoe UI", Font.BOLD, 40));
		lblNewLabel.setBounds(37, 84, 623, 107);
		...
		lblNewLabel_1.setBounds(91, 255, 521, 46);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					...
					fbal=GUI.balance+c.vald();
					st.executeUpdate("insert into transactions (accno,type,amount,balance,date) values("+GUI.accountno+",'Deposit',"+(c.vald())+","+fbal+",curdate())");
					st.executeUpdate("update account set balance="+fbal+" where accno="+GUI.accountno);
