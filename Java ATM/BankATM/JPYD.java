...
public class JPYD extends JFrame {
	/**
	 * 
	 */
	...
	static float jpy=(float) ((DepositInt.a)*0.56);
	...
	public JPYD() {
		...
		JLabel lblNewLabel = new JLabel("Entered Amount in INR is : ₹"+String.format("%.2f", jpy));
		...
		lblNewLabel_1.setBounds(89, 270, 525, 33);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					...
					st.executeUpdate("insert into transactions (accno,type,amount,balance,date) values("+GUI.accountno+",'Deposit',"+jpy+","+(GUI.balance+jpy)+",curdate())");
					st.executeUpdate("update account set balance="+(GUI.balance+jpy)+" where accno="+GUI.accountno);
