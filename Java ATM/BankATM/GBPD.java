...
public class GBPD extends JFrame {
	/**
	 * 
	 */
	...
	public GBPD() {
		...
		JLabel lblNewLabel = new JLabel("Entered Amount in INR is : ₹"+String.format("%.2f", gbp));
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					...
					st.executeUpdate("insert into transactions (accno,type,amount,balance,date) values("+GUI.accountno+",'Deposit',"+gbp+","+(GUI.balance+gbp)+",curdate())");
					st.executeUpdate("update account set balance="+(GUI.balance+gbp)+" where accno="+GUI.accountno);
